...
public record Config(bool ProcessNormalMessages = true, bool AddBotMeCommandInfo = true, bool AcceptMultiWordTriggers = false, bool CaseSensitive = false, bool CommandCaseSensitive = true) { }
public partial class TelegramBotCommandClient : TelegramBotClient, ITelegramBotClient
{
    ...
    public static string[] SeparateArgs(string input) => SeparateArgsRegex().Split(input).Where(s => !string.IsNullOrWhiteSpace(s)).ToArray();

    private readonly BotKey BotKey;
    private readonly Config Cfg;
    private readonly Func<Message, bool> MessageFilter;
    private readonly Dictionary<ChatId, IBotCommand> HeldCommands = new();
    ...
    public TelegramBotCommandClient(string token, int apiSaturation, BotKey key = BotKey.Any, Config? config = null, Func<Message, bool>? messageFilter = null, CommandProcessorUpdateHandler? updateHandler = null, HttpClient? client = null)
        : base(token, client)
    {
        Cfg = config ?? new();
        ...
        BotHandle = this.GetMyNameAsync().ConfigureAwait(false).GetAwaiter().GetResult().Name;
    }
    ...
    {
        var user = msg.From;
        Log.Debug("Message from user {user}, processing", user);
        if (!MessageFilter(msg))
        {
            Log.Debug("Message from user {user} filtered out", user);
            return;
        }
        ...
        try
        {
            if (command != null && (Cfg.ProcessNormalMessages || command.StartsWith("/")))
            {
                ...
                Log.Debug("Command {command} from user {user} succesfully processed.", command, user);
            }
        }
        catch (InvalidBotCommandException exc)
        {
            MessageQueue.EnqueueAction(b => b.SendTextMessageAsync(msg.Chat.Id, $"Invalid Command: {exc.Message}", replyToMessageId: msg.MessageId));
            Log.Debug($"Invalid Command {command} from user {user}");
        }
        catch (InvalidBotCommandArgumentsException exc)
        {
            MessageQueue.EnqueueAction(b => b.SendTextMessageAsync(msg.Chat.Id, $"Invalid Command Argument: {exc.Message}", replyToMessageId: msg.MessageId));
    ...
    private async Task<IEnumerable<BotAction>?> ReplyCall(BotCommandArguments args)
    {
        try
        {
            var (result, hold) = await HeldCommands[args.FromChat].ActionReply(args);
            if (!hold)
                HeldCommands.Remove(args.FromChat);
            return result;
        }
    ...
    public async Task<IEnumerable<BotAction>?> Call(BotCommandArguments args)
    {
        try
        {
            if (HeldCommands.ContainsKey(args.FromChat))
            ...
            if (hold)
                HeldCommands.Add(args.FromChat, cmd);
